...
export default async function EntityProfilePage({ params }: EntityProfilePageProps) {
  ...
  let entity;
  try {
    entity = await screeningService.getProjectEntity(projectId, projectEntityId);
  } catch (error) {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Tabs defaultValue="entity" className="space-y-6">
        <TabsList className="grid w-full grid-cols-2 lg:grid-cols-4">
          <TabsTrigger value="entity" className="flex items-center gap-1 lg:gap-2">
            <Book className="h-4 w-4" />
            <span>Summary</span>
          </TabsTrigger>
          <TabsTrigger value="matches" className="flex items-center gap-1 lg:gap-2">
            <SquareCheck className="h-4 w-4" />
            <span className="hidden sm:inline">Matches ({entity.matches?.length || 0})</span>
            <span className="sm:hidden">Matches</span>
          </TabsTrigger>
          <TabsTrigger value="supply-chain" className="flex items-center gap-1 lg:gap-2">
            <Waypoints className="h-4 w-4" />
            <span className="hidden sm:inline">Supply Chain (soon)</span>
            <span className="sm:hidden">Supply (soon)</span>
          </TabsTrigger>
          <TabsTrigger value="notes" className="flex items-center gap-1 lg:gap-2">
            <NotebookPen className="h-4 w-4" />
            <span className="hidden sm:inline">Notes & Recordkeeping (soon)</span>
            ...
            <Card>
              <CardContent className="pt-4 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <EntityTypeBadge type={entity.attributes?.type?.values?.[0] || 'unknown'} />
                  ...
                  {entity.strength && (
                    ...
                    >
                      {entity.strength === 'strong' ? 'high confidence' : 
                       entity.strength === 'partial' ? 'medium confidence' :
                       entity.strength === 'no_match' ? 'no match' : entity.strength}
                    </Badge>
                  )}
              ...
              <CardContent>
                <div className="flex items-center justify-center p-8">
                  <div className="text-center">
                    ...
                    <h3 className="text-lg font-medium mb-2">Workflow Status</h3>
              ...
              <CardTitle className="flex items-center gap-2">
                {hasRisks ? (
                  ...
                  <>
                    ...
                    Risk Assessment
                    <Badge variant="secondary" className="ml-2">Clear</Badge>
                  </>
                )}
              </CardTitle>
              <CardDescription>
              ...
              {hasRisks ? (
                ...
                <div className="flex items-center justify-center p-8">
                  <div className="text-center">
                    ...
                    <h3 className="text-lg font-medium mb-2">No Risk Factors Identified</h3>
                    <p className="text-sm text-muted-foreground">
                      This entity has no identified risk factors based on the current screening profile.
                    </p>
                  </div>
                </div>
              )}
            ...
            <CardContent>
              {hasUpstreamData ? (
                <div className="space-y-6">
                  {/* Upstream Countries */}
                  {entity.upstream?.countries && entity.upstream.countries.length > 0 && (
                    <div>
                      <h4 className="text-sm font-medium text-muted-foreground mb-3">Upstream Countries</h4>
                      <CountryBadgeList 
                  ...
                  {products.length > 0 && (
                    <div>
                      <h4 className="text-sm font-medium text-muted-foreground mb-3">Products (HS Codes)</h4>
                  ...
                  {entity.upstream?.risk_factors && Array.isArray(entity.upstream.risk_factors) && entity.upstream.risk_factors.length > 0 && (
                    <div>
                ...
                <div className="flex items-center justify-center p-8">
                  <div className="text-center">
                    ...
                    <h3 className="text-lg font-medium mb-2">No Supply Chain Data</h3>
                    <p className="text-sm text-muted-foreground">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between p-2 border rounded-lg">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-medium">compliance_report.pdf</p>
                          <p className="text-xs text-muted-foreground">2.3 MB • Uploaded 1 hour ago</p>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between p-2 border rounded-lg">
                      <div className="flex items-center gap-2">
                        <FileText className="h-4 w-4 text-muted-foreground" />
                        <div>
                          <p className="text-sm font-medium">risk_assessment.docx</p>
                          <p className="text-xs text-muted-foreground">1.8 MB • Uploaded 2 days ago</p>
                        </div>
                      </div>
                      <Button variant="ghost" size="sm">
                        <ExternalLink className="h-4 w-4" />
              ...
              <div className="space-y-4">
                <div className="border-l-4 border-blue-500 pl-4 py-2">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-sm font-medium">Initial Risk Assessment</h4>
                    <span className="text-xs text-muted-foreground">3 days ago</span>
                  </div>
                  <p className="text-sm text-muted-foreground mb-2">
                    Entity shows elevated risk factors related to sanctions exposure. Requires additional due diligence on beneficial ownership structure.
                  </p>
                  <div className="text-xs text-muted-foreground">
                ...
                <div className="border-l-4 border-yellow-500 pl-4 py-2">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-sm font-medium">Follow-up Investigation</h4>
                    <span className="text-xs text-muted-foreground">1 day ago</span>
                  </div>
                  <p className="text-sm text-muted-foreground mb-2">
                    Additional documentation received. Corporate structure verified through third-party sources. Risk level may be reduced pending final review.
                  </p>
                  <div className="text-xs text-muted-foreground">
